...
static double oi_getDistance(oi_t *self)
{
    ...
    int16_t leftEncoderDiff = self->leftEncoderCount - prevLeft;
    int16_t rightEncoderDiff = self->rightEncoderCount - prevRight;
    distLeft = (leftEncoderDiff) * (72 * M_PI / 508.8);
    distRight = (rightEncoderDiff) * (72 * M_PI / 508.8);
    prevLeft = self->leftEncoderCount;
    prevRight = self->rightEncoderCount;
...
/** 
 * @brief Returns the travelled distance of the wheels pulled directly from the OI packets
 * @author Armondo Reyes, Dawud Benedict
 * @return int wheel travel distance (in mm), since last call
 */
int getDistance()
{
    oi_uartSendChar(142);
...
/** 
 * @brief Returns the encoder value of the left wheel pulled directly from the OI packets
 * @author Armondo Reyes, Dawud Benedict
 * @return signed 2 byte int left wheel encoder (in mm), since last call
 */
int getLeftEncoder()
{
    oi_uartSendChar(142); // sensor data command code
    oi_uartSendChar(43); // left encoder packet ID
...
/** 
 * @brief Returns the encoder value of the right wheel pulled directly from the OI packets
 * @author Armondo Reyes, Dawud Benedict
 * @return signed 2 byte int right wheel encoder, since last call
 */
int getRightEncoder()
{
    oi_uartSendChar(142); // sensor data command code
    oi_uartSendChar(44); // right encoder packet ID
...
/** 
 * @brief Returns the calculated distance of the left wheel by using pulled encoder value
 * @author Armondo Reyes, Dawud Benedict
 * @return double distance left wheel traveled (in mm), since last call
 */
double getLeftDist()
{
    int left = getLeftEncoder();
    return (double) left * ((M_PI * 72.0) / 508.8);
}

/** 
 * @brief Returns the calculated distance of the right wheel by using pulled encoder value
 * @author Armondo Reyes, Dawud Benedict
 * @return double distance right wheel traveled (in mm) since last call
 */
double getRightDist()
{
    int right = getRightEncoder(); // pull the right encoder data for the right wheel
    return (double) right * ((M_PI * 72.0) / 508.8);
}

/** 
 * @brief Returns the calculated angle change in the wheels
 * @author Armondo Reyes, Dawud Benedict
 * @return signed 2 byte int angle (in degrees), since last call
 */
int getAngle()
{
    oi_uartSendChar(142);
...
/** 
 * @brief Returns the signal of the left cliff sensor, for detecting black and white surfaces
 * @author Armondo Reyes, Dawud Benedict
 * @return unsigned 2 byte int cliff signal
 */
int getCliffLeft()
{
    oi_uartSendChar(142);
...
/** 
 * @brief Returns the signal of the left front cliff sensor, for detecting black and white surfaces
 * @author Armondo Reyes, Dawud Benedict
 * @return unsigned 2 byte int cliff signal
 */
int getCliffLeftFront()
{
    oi_uartSendChar(142);
    oi_uartSendChar(29);
...
/** 
 * @brief Returns the signal of the right cliff sensor, for detecting black and white surfaces
 * @author Armondo Reyes, Dawud Benedict
 * @return unsigned 2 byte int cliff signal
 */
int getCliffRight()
...
/** 
 * @brief Returns the signal of the right front cliff sensor, for detecting black and white surfaces
 * @author Armondo Reyes, Dawud Benedict
 * @return unsigned 2 byte int cliff signal
 */
int getCliffRightFront()
{
    oi_uartSendChar(142);
    oi_uartSendChar(30);
...
/** 
 * @brief Reads left bumper sensor to check if anything has been hit
 * @author Armondo Reyes, Dawud Benedict
 * @return boolean int, 1 = hit 0 = no hit
 */
int getLeftBump()
{
    oi_uartSendChar(142);
    ...
    int data = (oi_uartReceive() & 0x2) >> 1;

    return data;
}

/** 
 * @brief Reads right bumper sensor to check if anything has been hit
 * @author Armondo Reyes, Dawud Benedict
 * @return boolean int, 1 = hit 0 = no hit
 */
int getRightBump()
{
    ...
    int data = oi_uartReceive() & 0x1;

    return data;
}

/** 
 * @brief manually calculates the bots angle change by pulling the wheel encoders and converting from radians
 * @author Armondo Reyes, Dawud Benedict
 * @return double degree turn of the bot, since last call
 */
double get_angle_degrees()
{
    ...
    int leftEncoderCount = getLeftEncoder();
    int rightEncoderCount = getRightEncoder();

    if ((leftEncoderCount == prevLeft) && (rightEncoderCount == prevRight))
    {
        prevLeft = leftEncoderCount;
        prevRight = rightEncoderCount;
        return 0;
    }
    // ignore the first run, such that we do not have prevLeft or right=0, this
    // would give a very large degree moved.
    ...
    // get distance moved in mm for the left and right wheel
    // equation: ticks * (1/508.8)*72pi
    int16_t leftEncoderDiff = leftEncoderCount - prevLeft;
    int16_t rightEncoderDiff = rightEncoderCount - prevRight;
    // 508.8 encoder ticks per wheel revolution, wheel is 72π mm diameter
    double distLeft = (leftEncoderDiff) * (72.00 * M_PI / 508.8);
    double distRight = (rightEncoderDiff) * (72.00 * M_PI / 508.8);
    prevLeft = leftEncoderCount;
    ...
    // Radians = (distanceRight - distanceLeft) / wheel-base (per datatsheet)
    double radians = (distRight - distLeft) / 235.00;
    double degrees = radians * 180.00 / M_PI;
    return degrees;
}

/** 
 * @brief prepares the game over sound from the Super Mario games
 * @author Armondo Reyes, Dawud Benedict
 */
void prepMarioSound()
{
    oi_uartSendChar(140);
    oi_uartSendChar(0); //song 0
    oi_uartSendChar(12); // song length
    oi_uartSendChar(67); // note 1 D
    oi_uartSendChar(9);
    oi_uartSendChar(74); // note 2 G
    oi_uartSendChar(9);
    oi_uartSendChar(31); // note 3 Rest
    oi_uartSendChar(9);
    oi_uartSendChar(74); //note 4 G
    oi_uartSendChar(9);
    oi_uartSendChar(74); //note 5 G half
    oi_uartSendChar(12);
    oi_uartSendChar(72); //note 6 A half
    oi_uartSendChar(12);
    oi_uartSendChar(71); //note 7 B half
    oi_uartSendChar(12);
    oi_uartSendChar(67); //note 8 D
    oi_uartSendChar(9);
    oi_uartSendChar(64); //note 9 F
    oi_uartSendChar(9);
    oi_uartSendChar(31); //note 10 rest
    oi_uartSendChar(9);
    oi_uartSendChar(64); // note 11 F
    oi_uartSendChar(9);
    oi_uartSendChar(60); //note 12 A
    oi_uartSendChar(9);

}

/** 
 * @brief Plays a prepared song with an input song number (only Mario sound)
 * @author Armondo Reyes, Dawud Benedict
 */
void playSong(int song_num)
{
    oi_uartSendChar(141);
    oi_uartSendChar(song_num);
}
